...
import ru.kvanttelecom.tv.streammonitoring.utils.converter.CameraUpdateConverter;
import ru.kvanttelecom.tv.streammonitoring.utils.data.Stream;
import ru.kvanttelecom.tv.streammonitoring.utils.data.CameraState;
...
public class CameraService {
    ...
    private static final Double CAMERA_FLAPPING_MIN_RATE = 1./600; // 1 раз в 10 мин
     ...
     */
    public List<StreamEvent> applyUpdate(String serverName, List<CameraUpdate> updateList) {
        ...
        try {
            ...
            for (CameraUpdate update : updates.values()) {
                ...
                if (stream == null) {
                    ...
                    if (serverHasCameras) {
                        ...
                        log.debug("{} CAMERA: {} {}",typeList, stream.getName(), stream.getTitle());
                    }
                    // INIT CAMERA
                    else {
                    ...
                    StreamEvent streamEvent = new StreamEvent(cameraName, typeList);
                ...
                // CAMERA STATUS CHANGED - existing camera found - check camera status modifications
                    ...
                    Set<CameraEventType> typeList = calcCameraAlternation(stream, update);

                    // If camera status has been changed
                    if (typeList.size() > 0) {
                        StreamEvent event = new StreamEvent(cameraName, typeList);
                        result.add(event);
            ...
            // DELETED CAMERA
            for (Map.Entry<String, Stream> entry : serverCameras.entrySet()) {
     ...
     */
    private Set<CameraEventType> calcCameraAlternation(Stream stream, CameraUpdate update) {

        Set<CameraEventType> result = new HashSet<>();

        CameraState state = stream.getState();
        ...
        if(flappingRate != null) {
            ...
            if(!stream.isFlapping() && flappingRate > cameraFlappingMinRate) {
        ...
        if(stream.isAlive() && state.getLevel() < -CAMERA_THRESHOLD_LEVEL) {
