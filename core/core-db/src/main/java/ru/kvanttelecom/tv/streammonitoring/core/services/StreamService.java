...
@Transactional
@Slf4j
public class StreamService extends BaseRepoAccessService<Stream> {

    private final StreamRepository repository;
    public final IMap<StreamKey, Stream> map;
    private final HazelcastInstance instance;

    @Autowired
    public StreamService(StreamRepository repository, @Qualifier("myHazelcast") HazelcastInstance instance) {
        super(repository);
        this.repository = repository;
        this.instance = instance;

        Config config = instance.getConfig();
        String mapName = MethodHandles.lookup().lookupClass().getSimpleName();
    ...
    public Optional<Stream> findByKey(StreamKey key) {
        ...
        if(map.containsKey(key)) {
            result = Optional.of(map.get(key));
        }
        else {
            result = repository.findOneByStreamKey(key, null);
            result.ifPresent(stream -> map.put(key, stream));
        }
        return result;
    }
